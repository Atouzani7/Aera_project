...
export class ProjectService {
  ...
  async createProject(
    ...
    if (dto.clientId) {
      clientEntity = await this.clientService.findOne(dto.clientId);
    ...
    // ✅ Cas 2 : email fourni
      ...
      // Recherche par email
      clientEntity = await this.clientService.findClientByEmail(
        dto.contact_email,
      );

      // Si aucun client → création
      if (!clientEntity) {
        clientEntity = await this.clientService.create(
          {
            ...
            country: dto.contact_country ?? 'France',
    ...
    // ❌ Aucun client fourni
      ...
      throw new BadRequestException(
        'Vous devez fournir soit clientId soit contact_email',
      );
    }
